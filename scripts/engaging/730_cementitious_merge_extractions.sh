#!/bin/bash
# MIT Engaging — verify + merge all extraction shards.
set -euo pipefail
...
python -m pipeline.cementitious.cluster merge-extract --output "$OUT"
echo "extract merge complete -> $OUT/checkpoints/extract_merge.complete"
